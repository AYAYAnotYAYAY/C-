// 堆栈计算器.cpp: 定义控制台应用程序的入口点。
//
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
struct yuansu { char data; double num; yuansu *next; yuansu *father; };//定义一个链表结构的数据类型，用来做栈计算

struct yuansu *pop(yuansu *R)//出栈，返回next
	{
	yuansu *temp;
	temp = R->father;
	free(R);
...
struct yuansu *push( yuansu *zz, char a,double i)//入栈，返回新地址
	{
	yuansu *temp;
...
double sum(yuansu *S)//提供后缀表达式链表，计算最终结果
{
	yuansu *H;//计算时用到的计算栈
	H= (yuansu*)malloc(sizeof(struct yuansu));//初始化第一个地址
	while (S!=NULL)
	{
...
int panduan(yuansu *S,char suanshi)//比较提供的链表中的符号情况，决定参数符号是否应该直接入栈。应该直接入栈返回1，不应该返回0,如是右括号连续输出直到左括号为止返回2.
{
...
struct yuansu *zhuanhuan(char suanshi[])//提供字符串类型的中缀表达式转换为双项链表结构的后缀表达式
{
	yuansu *F, *S,*r;
	double h = 0;
	F = (yuansu*)malloc(sizeof(struct yuansu));//正式后缀表达式链表
	r = F;
	F->father = NULL;
	F->next = NULL;
	S = (yuansu*)malloc(sizeof(struct yuansu));//运算符号堆栈链表
	S->father = NULL;
